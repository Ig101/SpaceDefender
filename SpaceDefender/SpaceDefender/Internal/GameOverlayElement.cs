    ...
    class GameOverlayElement: SpriteElement
    {
        ...
        public override void Draw(IgnitusGame game, Matrix animation, Color fonColor, float milliseconds)
        {
            ...
            if (coff > 0)
            {
                if (shown)
                {
                    game.DrawSprite(SpriteName, new Rectangle(X, Y, Width, Height), Source, new Color((byte)(Color.R * fonColor.R / 255 * coff),
                        (byte)(Color.G * fonColor.G / 255 * coff), (byte)(Color.B * fonColor.B / 255 * coff), (byte)(Color.A * fonColor.A / 255 * coff)), 0, Vector2.Zero,
